...
describe '#my_each_with_index' do
          ...
          it 'check if it is accepting range' do
         expect(range.my_each_with_index { |num, index| num > 2}).to eql(range)
          end
         it 'check if it is accepting hash' do
        expect(hash.my_each_with_index { |num, index| num}).to eql(hash)
         end
          it 'when block is not given return Enumerator' do
    ...
    describe '#my_select' do
        ...
        expect(array.my_select { |num| num < 3}).to eql([0, 1, 2])
        end

        it 'returns an array into a hash' do
        expect(array.my_select { |key, value| value < 2}).to eql({ c: 3})
        end
    end
    describe '#my_all' do
        ...
        expect(hash.my_all? { |key, value| value > 7 }).to eql TRUE

        it 'return false when one value is false in a range' do
        expect(hash.my_all? { |key, value| value > 3 }).to eql TRUE
    end
    describe '#my_any?' do
        ...
        expect(range.my_any? { |key, value| value > 2}  ).to eql TRUE
        end

        it 'return false when no value is true on a hash' do
        expect(range.my_any? { |key, value| value > 6}  ).to eql false
        end
    end
